...
For orders, it uses keyword matching and regex to extract five fields: item, flavor, quantity, pickup date, and pickup time. These are stored in session state and merged across turns so that a customer can provide information gradually and the bot can keep track of what it has and what it still needs.
